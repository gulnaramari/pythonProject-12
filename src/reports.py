...
from src.utils import mod_date
...
def spending_by_category(transactions: pd.DataFrame, category: str, date: Optional[str] = None) -> pd.DataFrame:
    ...
    try:
        ...
        grouped_transactions = transactions.loc[
            (pd.to_datetime(transactions["Дата операции"], dayfirst=True) <= fin_date)
...
if __name__ == "__main__":
    result = spending_by_category(config.transactions, "Супермаркеты", "31.12.2021 15:44:39")
    print(result)
